...
## Status: steps 4 and 7 — weighted picker, refresh + background sync
...
`lib/grid.ts`). A Refresh button above the grid syncs on demand and shows "synced X ago";
...
src/
  ...
  lib/
    ...
    grid.ts          weighted picker (which videos the grid shows) + duration/time formatting
...
7. **Manual refresh + `chrome.alarms` timer** ✓ (done ahead of 5–6)
